...
public class NewInstallUserRunner implements Tool {
    ...
    private void calculateTotalUsers(Configuration conf) {
        ...
        try {
            ...
            // 先查询昨天的总注册用户
            /**
             * 这里把platform_dimension_id也查出来的原因是：stats_user 的主键是 date_dimension_id 和 platform_dimension_id
             * 要根据日期id和平台id来更新总用户数
             *
             * 下面的 stats_device_browser 表 同理
             */
            ...
            while (rs.next()) {
                ...
                if (oldValueMap.containsKey("" + platformId)) {
                    newUsers += oldValueMap.get("" + platformId);  //在这里进行的+=操作，更新，
                }
