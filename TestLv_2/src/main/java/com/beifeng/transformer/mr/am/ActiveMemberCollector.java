...
public class ActiveMemberCollector implements IOutputCollector {
    ...
    public void collect(Configuration conf, BaseDimension key, BaseStatsValueWritable value, PreparedStatement pstmt, IDimensionConverter converter) throws SQLException, IOException {
        ...
        //如果是统计browser active member的，那么就再加一个sql语句中的？占位符，
        //和active_user相比，也就是把ActiveUserBrowserCollector和ActiveUserCollector 合并了，也就不用再单独定义一个ActiveMemberBrowserCollector
        //可以少写一个类，然后在output-collector.xml中定义全类名的时候，定义collector_browser_active_member的时候和collector_active_member一样即可
