...
public class NewMemberMapper extends TableMapper<StatsUserDimension, TimeOutputValue> {
    ...
    //输出key StatsUserDimension 维度中的通用维度中的kpi基本维度维度
    //因为要输出到两种表，所以：两个kpi维度，一个是用户基本信息分析中的新会员，一个是浏览器信息分析中的新会员；
    ...
    protected void setup(Context context) throws IOException, InterruptedException {
        ...
        try {
            ...
            // 删除指定日期的数据
            /**
             * 防止mr程序重跑时，由于上一次跑已经把所有新增的member id已经加入到了member info中，所以再次重跑造成应有的新增用户都不被判断为新增用户的错误
             */
