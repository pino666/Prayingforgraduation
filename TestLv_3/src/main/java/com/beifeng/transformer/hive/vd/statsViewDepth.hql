...
CREATE EXTERNAL TABLE event_logs(row string, pl string, en string, s_time bigint, p_url string, u_ud string, u_sd string)
...
-- 6. 创建hive的function
create function platform_convert as 'com.beifeng.transformer.hive.PlatformDimensionUDF' using jar 'hdfs://hh:8020/beifeng/transformer/transformer-0.0.1.jar';  
create function date_convert as 'com.beifeng.transformer.hive.DateDimensionUDF' using jar 'hdfs://hh:8020/beifeng/transformer/transformer-0.0.1.jar';  


-- 7. hql编写(统计用户角度的浏览深度) 注意：两次groupby操作 <注意：时间为外部给定>
from (
