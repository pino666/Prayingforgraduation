...
public abstract class TransformerBaseRunner implements Tool {
    ...
    private boolean isCallSetUpRunnerMethod = false;   //相当于一个标志位。作用是要求子类必须先设置上述类型，否则job执行的时候直接报错；
     ...
     * @param reducerClass
     *            reducer的class
     * @param outputKeyClass    =    outputKeyClass     也就是说map 的输出类型就是  job的最终输出类型
     *            输出key
     * @param outputValueClass    =  outputValueClass
     *            输出value
     */
     ...
     * @param mapperClass
     * @param reducerClass
     * @param outputKeyClass        =    outputKeyClass
     * @param outputValueClass      =    outputValueClass
     ...
     * @param mapperClass
     * @param reducerClass
     * @param mapOutputKeyClass        map的输出key类型
     * @param mapOutputValueClass      map的输出value类型
     * @param outputKeyClass           job的输出key类型
     * @param outputValueClass         job的输出value类型
     * @param outputFormatClass
     */
    public void setupRunner(String jobName, Class<?> runnerClass, Class<? extends TableMapper<?, ?>> mapperClass, Class<? extends Reducer<?, ?, ?, ?>> reducerClass, Class<? extends WritableComparable<?>> mapOutputKeyClass, Class<? extends Writable> mapOutputValueClass, Class<? extends WritableComparable<?>> outputKeyClass, Class<? extends Writable> outputValueClass,
